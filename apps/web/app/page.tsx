...
export default function Home() {
  ...
  async function reviewGithubPr(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNotice("");
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"}/api/v1/reviews/from-github`, {
        method: "POST",
  ...
  return (
    <main className="shell">
      ...
      <section className="content">
        ...
        <section className="import-card"><div className="import-copy"><div className="import-icon"><GitPullRequest size={17} /></div><div><strong>Review a real GitHub pull request</strong><p>Paste a public PR URL and ReviewPilot will fetch its diff and scan it.</p></div></div><form className="import-form" onSubmit={reviewGithubPr}><input aria-label="GitHub pull request URL" value={prUrl} onChange={(event) => setPrUrl(event.target.value)} placeholder="https://github.com/owner/repo/pull/123" required /><button className="primary-button" type="submit" disabled={isRunning}>{isRunning ? "Analyzing…" : "Analyze PR"}<ArrowUpRight size={15} /></button></form></section>
